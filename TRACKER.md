...
<!-- Fresh start from Chapter 1: this list is empty, so the completion gate is CLEAR and
     the next run produces the FIRST batch (Chapter 1 of each book plus AWS topic 1).
     Font paths now resolve independently of the working directory, so slide text wraps
     correctly instead of overflowing. It then writes the new checklist here to tick. -->
