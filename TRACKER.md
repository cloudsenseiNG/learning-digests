...
<!-- Fresh start from Chapter 1: this list is empty, so the completion gate is CLEAR and
     ...
     the current signature carousel style, then writes the new "Current batch" checklist
     here for you to tick. Nothing new arrives after that until every box is checked. -->
